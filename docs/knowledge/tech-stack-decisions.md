> **Canonical copy:** [`docs/org-conventions/tech-stack-decisions.md`](../org-conventions/tech-stack-decisions.md) — update there, not here.
...
See `vercel-deploy-status.md` for current URL/domain issues.
...
**Status:** Not connected on Vercel production yet — see `vercel-deploy-status.md`.
...
From `.env.example` + project usage. **Never commit real values.**
...
**Local state (2026-08-08):** `NTFY_TOPIC`, Twilio vars, `OPENAI_API_KEY`, and `ADMIN_UPLOAD_KEY` are set in `.env.local`. `DATABASE_URL` is **not** set locally or on Vercel.
