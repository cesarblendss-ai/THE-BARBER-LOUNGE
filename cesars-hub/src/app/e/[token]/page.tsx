...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!estimate) {
    return { title: `Estimate — ${HUB.name}`, robots: { index: false, follow: false } };
  }

  return {
    title: `Estimate for ${estimate.clientName} — ${HUB.name}`,
...
export default async function PublicEstimatePage({ params, searchParams }: PageProps) {
  ...
  return (
    <section className="bg-bone px-4 pb-16 pt-16 sm:px-6">
